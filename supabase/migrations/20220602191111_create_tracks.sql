...
create trigger handle_updated_at before update on tracks
...
create policy "Tracks are viewable by users who created them."
...
create or replace function track_start(multiplier double precision default 1.0)
  returns setof tracks
as $$
  insert into tracks (tracked, multiplier, account)
  values (tstzrange(now(), NULL), $1, auth.uid()) returning *;
$$
language sql;

create or replace function track_stop(id uuid)
  returns setof tracks
as $$
  update tracks set tracked = tstzrange(lower(tracked), now())
